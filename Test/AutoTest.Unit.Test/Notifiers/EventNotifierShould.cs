...
public class EventNotifierShould
{
    ...
    public async Task Notify()
    {
        ...
        clientProxy.Setup(a => a.SendCoreAsync("NewNotification", new[] { notification }, TestContext.Current.CancellationToken)).Returns(Task.CompletedTask).Verifiable(Times.Once);
        clients.Setup(a => a.Group($"eventId:{eventId}")).Returns(clientProxy.Object).Verifiable(Times.Once);
    ...
    public async Task NewTestRun()
    {
        ...
        clientProxy.Setup(a => a.SendCoreAsync("NewResults", new[] { results }, TestContext.Current.CancellationToken)).Returns(Task.CompletedTask).Verifiable(Times.Once);
        clientProxy.Setup(a => a.SendCoreAsync("NewTestRun", new[] { testRun }, TestContext.Current.CancellationToken)).Returns(Task.CompletedTask).Verifiable(Times.Once);
        clients.Setup(a => a.Group($"eventId:{eventId}")).Returns(clientProxy.Object).Verifiable(Times.Once);
        _eventHub.Setup(a => a.Clients).Returns(clients.Object).Verifiable(Times.Once);
        _mediator.Setup(a => a.Send(Its.EquivalentTo(new GetResults(eventId)), TestContext.Current.CancellationToken)).ReturnsAsync(results).Verifiable(Times.Once);
